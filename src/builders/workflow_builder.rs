...
use crate::model::{state::State, variable_mapping::VariableMapper, workflow::{CommandRunner, Workflow}};
...
impl<C: CommandRunner, M: VariableMapper> WorkflowBuilder<C, M> {
    ...
    pub fn with_initial_state(&mut self, initial_state: &Rc<State<Workflow<C, M>, M>>) -> &mut Self {
    ...
    pub fn with_initial_command_output_to_display(&mut self, initial_command_output_to_display: M) -> &mut Self {
        self.initial_command_output_to_display = Some(initial_command_output_to_display);
        self
    }

    pub fn build_initial_state(&mut self, f: impl FnOnce(StateBuilder<Workflow<C, M>, M>) -> State<Workflow<C, M>, M>) -> &mut Self {
    ...
    pub fn build(&self) -> Workflow<C, M> {
        let mut workflow = Workflow::new(self.command_runner.clone().expect("Command runner is required to build Workflow"));
        if let (
                Some(initial_state),
            ...
            workflow.init(Rc::clone(initial_state), initial_command, initial_command_output_to_display.clone());
        }
        workflow
    }
}